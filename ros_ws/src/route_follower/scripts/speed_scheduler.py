#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Speed Scheduler v2: 预测/动力学可行性限速 (转弯爬升骤降速修复方案 1~16, 33 节)。

只保留三类限制:
  v_curve   = sqrt(a_lat_max / (|κ|+ε))        真实横向加速度能力
  v_slope   = slope_eta*vz_up_safe / |dz/ds|    真实垂直速度能力
  v_tracking= 预测穿门误差 / Z 掉队(且恶化) / Yaw 误差  -> 软限制

hard_cap = min(cruise, v_curve, v_slope)        物理硬限制, 可低于 floor
soft_cap = max(normal_speed_floor, v_tracking)  软限制不得低于 floor
v_target = min(hard_cap, soft_cap)

删除 gate_bad_near 的"瞬时误差 -> 1.5 m/s"和 extra_cap(坡度重复限速)。
速度斜坡区分 a_up / a_down。
"""
...
class SpeedScheduler(object):
    ...
    def __init__(self, cruise_speed=10.0, max_speed=12.0,
                 ...
                 slope_eta=0.8, vz_up_safe=3.0, a_up=4.0, a_down=5.0,
                 z_slow1=0.4, z_slow2=0.8, z_slow3=1.2,
                 z_f2=0.8, z_f3=0.6, z_f4=0.4,
    ...
    def slope_limit(self, kz, trusted=True):
        v = self.slope_eta * self.vz_up_safe / (abs(kz) + 1e-3)
        if not trusted:                 # 不可信(Soft)坡度不得把速度压到 floor 以下
            v = max(v, self.floor)
        return min(self.cruise, v)

    def tracking_limit(self, miss_ratio, z_err, z_worsening, pred_worse, yaw_err_deg):
        """返回软限速 (未与 floor 取 max)。"""
        f = 1.0
        ...
        az = abs(z_err)
        if az >= self.z_slow3 and pred_worse:                 # 接近安全边界且恶化
            f = min(f, self.z_f4)
        elif az >= self.z_slow2 and (z_worsening or pred_worse):
            f = min(f, self.z_f3)
    ...
    def target(self, kap, kz, slope_trusted, miss_ratio, z_err, z_worsening,
        ...
        v_slope = self.slope_limit(kz, slope_trusted)
